...
- credential storage on disk (`credentials.json`, `edge_session.json`)
...
- JWT access or refresh tokens from `credentials.json`
...
- never committing `data/credentials.json` or `data/edge_session.json`
- using strong local credentials for cloud accounts
- avoiding exposing the agent's control-plane port (`8001` by default) to the public network
