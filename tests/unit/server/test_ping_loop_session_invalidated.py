...
import pytest
from sellerclaw_agent.cloud.exceptions import CloudSessionInvalidatedError
...
async def test_ping_loop_session_invalidated_clears_session_keeps_creds(
    monkeypatch,
    tmp_path,
) -> None:
    """401 agent_session_invalidated: clear only the session, keep the agent_token."""
    ...
    mock_client.ping = AsyncMock(
        side_effect=CloudSessionInvalidatedError("stale session", status_code=401),
    )
    ...
    assert sleep_calls, "expected session-invalidated backoff sleep"
