...
- `heartbeat` — heartbeat policy, cloud-owned because the cloud also owns `HEARTBEAT.md`: `every` (cadence; `"0m"` disables the periodic poll) and `target` (where a heartbeat run's output goes; `"none"` keeps it internal). Both are written into `agents.defaults.heartbeat`, and both default to the quiet value when absent — never to OpenClaw's built-in defaults, which are an enabled hourly poll delivering into the owner's own chat. Not to be confused with `content.heartbeat`, which is the text of `HEARTBEAT.md`.
